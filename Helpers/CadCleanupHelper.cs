...
namespace Cad2Revit.Helpers
{
    public static class CadCleanupHelper
    {
        ...
        public static int XoaTatCaLevelTru0(Document doc)
        {
            if (doc == null)
                return 0;
                ...
                .Cast<Level>())
            {
                ...
                if (Math.Abs(l.Elevation - 0) < tolFeet)
                    continue;
        ...
        public static int XoaTatCaLevelKhongPhaiCad2Revit(Document doc)
        {
            if (doc == null)
                return 0;
                ...
                .Cast<Level>())
            {
                ...
                if (l.Name.StartsWith("CAD2Revit", StringComparison.OrdinalIgnoreCase))
                    continue;
        ...
        public static int XoaTatCaCauKien(Document doc)
        {
            if (doc == null)
                return 0;
            ...
            ids.AddRange(LayTatCaCauKienTheoCategory(doc, BuiltInCategory.OST_StructuralColumns));
            ids.AddRange(LayTatCaCauKienTheoCategory(doc, BuiltInCategory.OST_Columns));
            ids.AddRange(LayTatCaCauKienTheoCategory(doc, BuiltInCategory.OST_StructuralFraming));
            ids.AddRange(LayTatCaCauKienTheoCategory(doc, BuiltInCategory.OST_Floors));
            ids.AddRange(LayTatCaCauKienTheoCategory(doc, BuiltInCategory.OST_Walls));
        ...
        /// <summary>
        /// Xóa các Level mặc định thường có trong template (Level 1 / Level 2, Tầng 1 / Tầng 2).
        /// Dùng khi muốn loại bỏ các level template mặc định trước khi tạo lại mô hình.
        /// </summary>
        public static int XoaLevelMacDinh(Document doc)
        {
            if (doc == null)
                return 0;
            ...
            // Sử dụng regex để bắt các tên mẫu như 'Level 1', 'Tầng 1', 'Level01', v.v.
            var rx1 = new System.Text.RegularExpressions.Regex(@"^(?:\s*(?:tầng|tang|level)\s*0*1\s*)$", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
            var rx2 = new System.Text.RegularExpressions.Regex(@"^(?:\s*(?:tầng|tang|level)\s*0*2\s*)$", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                ...
                .Cast<Level>())
            {
                ...
                if (rx1.IsMatch(name) || rx2.IsMatch(name))
                {
                    ids.Add(l.Id);
                }
            }
